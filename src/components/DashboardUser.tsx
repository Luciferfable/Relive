...
export default function DashboardUser({
  ...
  const [chatMessages, setChatMessages] = useState<ChatMessage[]>([
    {
      id: 'welcome',
      role: 'model',
      text: "Namaste, Aarav Sharma! I am your scientific AI archival assistant. I can diagnose your old family cassette tapes, detail the custom 4K digitization scanning workflow, or check the status of your जयपुर (Jaipur) pickup couriers. What childhood memories are we exploring today?",
  ...
  const [authWarningType, setAuthWarningType] = useState<'drive' | 'photos' | null>(null);
  ...
  const handleConnectPhotos = async () => {
    try {
      ...
      if (e?.code === 'auth/popup-closed-by-user' || e?.message?.includes('popup-closed-by-user') || e?.message?.includes('closed by user') || e?.message?.includes('popup')) {
          ...
          try {
            ...
            if (res.ok) {
              ...
              if (data && data.display_name) {
                setNewAppt(prev => ({ ...prev, address: data.display_name }));
                setIsCapturingLoc(false);
                return;
              }
            }
  ...
  const handleTabSendVerificationEmail = async () => {
    ...
    try {
      ...
      if (data.success) {
        ...
        setTabVertError("Validation dispatch rejected.");
      }
    } catch (e: any) {
      setTabVertError("API connection failure.");
    } finally {
      setTabVertLoading(false);
    }
  };

  const handleTabVerifyCode = () => {
    if (tabUserVertPIN.trim() === tabVertPIN && tabVertPIN !== '') {
      ...
      setTabVertError("Invalid credentials code pin. Please retry!");
    }
  };
  ...
  const handleChangePassword = async (e: React.FormEvent) => {
    ...
    try {
      if (auth.currentUser) {
        ...
        console.warn("Updating local credentials inside sandbox security envelope...");
        setPasswordMsg("✓ Sandbox simulated profile passcode upgraded successfully!");
      }
      setCurPassword('');
      setNewPasswordState('');
      setConfirmPassword('');
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center bg-stone-900 text-stone-100 p-6 sm:p-8 rounded-3xl border border-stone-800 shadow-xl relative overflow-hidden">
        ...
        <div className="space-y-2 z-10">
          ...
          <div className="flex items-center gap-3">
            <h1 className="text-2xl sm:text-3xl font-serif">Namaste, Aarav Sharma</h1>
            <span className="px-2.5 py-0.5 bg-amber-500/20 text-amber-300 text-xs rounded-full border border-amber-500/30">C-Scheme, Jaipur</span>
          </div>
          <p className="text-stone-400 text-xs sm:text-sm">Account UID: user-01 • Verified Member since 2026</p>
        </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
            ...
            <div className="lg:col-span-4 space-y-6">
              ...
              <div className="bg-white border border-stone-200 rounded-2xl p-6 shadow-xs space-y-4 text-center">
                ...
                <div>
                  <h3 className="font-serif text-base font-semibold text-stone-950">Aarav Sharma</h3>
                ...
                <div className="text-left text-[11px] text-stone-500 space-y-1">
                  <p>📍 Address: 12, Heritage Lane, C-Scheme, Jaipur</p>
                  <p>📞 Phone PIN: +91 98765 43210</p>
                </div>
              </div>
            ...
            <div className="flex flex-col gap-4 bg-stone-100 p-4 rounded-xl">
              <div className="flex flex-col lg:flex-row justify-between items-start lg:items-center gap-4">
                ...
                <div className="flex gap-2 text-xs shrink-0 w-full lg:w-auto justify-end">
                  <button
                    id="zip-download-all"
                    onClick={() => alert("Packaging all high-definition restored tiff/mp4 records to ReLive_Archival_Aarav.zip (Size: 12.4MB)")}
                ...
                return (
                  <div key={file.id} className="bg-white border border-stone-200 rounded-2xl overflow-hidden hover:shadow-md transition-all flex flex-col justify-between">
                    <div className="relative aspect-video bg-stone-950 overflow-hidden group">
                      ...
                      {locked && (
                        <div className="absolute inset-0 bg-stone-900/70 backdrop-blur-xs flex flex-col items-center justify-center text-center p-4 z-10">
                          ...
                          <span className="text-[10px] text-stone-300">Requires Payment (Order #{file.orderId})</span>
                        </div>
                      )}
          ...
          <div className="fixed inset-0 bg-stone-950/80 backdrop-blur-md flex items-center justify-center z-50 p-4">
            ...
            >
              ...
              <div className="flex gap-3 items-center border-b border-stone-800 pb-3">
                ...
                <div>
                  <h3 className="font-serif text-lg text-amber-500 font-extrabold">Google Handshake Restrained</h3>
                  <p className="text-stone-400 text-[10px] font-mono uppercase tracking-wider">Error Module: auth/popup-closed-by-user</p>
                </div>
              </div>
            ...
            >
              ...
              <div className="bg-stone-50 p-4 rounded-xl text-left border border-stone-200/80 font-mono text-[11px] space-y-2 text-stone-700">
                ...
                <div className="flex justify-between">
                  ...
                  <span className="font-bold">{checkoutSuccess.userName || 'Aarav Sharma'}</span>
                </div>
                <div className="flex justify-between">
