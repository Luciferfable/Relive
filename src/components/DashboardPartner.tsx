...
export default function DashboardPartner({ orders, onUpdateOrder }: DashboardPartnerProps) {
  ...
  const currentTaskOrder = orders.find(o => o.assignedPartnerId === 'partner-delhi' && o.deliveryStatus !== 'delivered');
  ...
  const markArrivedAtDoorstep = () => {
    ...
    alert("Marked as Arrived! Notification dispatched to customer Aarav Sharma.");
  };
  ...
  const verifyOtpCode = (e: React.FormEvent) => {
    ...
    if (typedOtp === currentTaskOrder.pickupOtp) {
      onUpdateOrder({
        ...currentTaskOrder,
      ...
      <div className="bg-stone-900 text-stone-100 p-5 rounded-3xl border border-stone-850 shadow-xl flex items-center justify-between">
        <div className="flex items-center gap-3">
          <div className="w-12 h-12 rounded-full overflow-hidden border-2 border-amber-400">
            <img src="https://images.unsplash.com/photo-1620122303020-43ec4b6cf7f8?w=120&q=80" alt="Kartik" className="w-full h-full object-cover" />
          </div>
          <div>
            <h1 className="font-serif font-black text-sm sm:text-base text-stone-50">Kartik Yadav</h1>
            <p className="text-[10px] text-amber-400">ReLive Jaipur Logistics Leader</p>
          </div>
        </div>
          ...
          <div className="space-y-4 text-xs">
            <div className="bg-stone-50 p-4 rounded-2xl border space-y-3">
              <div className="flex justify-between font-bold text-stone-900 border-b pb-2">
                ...
                <span className="text-amber-800">{currentTaskOrder.deliveryStatus.toUpperCase()}</span>
              </div>
              ...
              <div className="space-y-1.5 font-light text-stone-700">
                ...
                <p className="flex items-center gap-1.5 bg-amber-500/10 p-2 rounded-xl border border-amber-500/20 text-[11px] text-amber-900 my-1.5 font-sans">
                  <Phone className="w-3.5 h-3.5 text-amber-800 shrink-0" />
                  <span className="font-bold">Customer Contact No:</span>
              ...
              <div className="bg-stone-950 text-stone-100 rounded-3xl p-5 border border-stone-800 space-y-4 animate-fade-in">
                ...
                <div className="bg-stone-900 border border-stone-800 p-3 rounded-xl space-y-2">
                  <div className="flex items-center justify-between text-[11px]">
                    ...
                    <span className="text-amber-400 font-bold font-mono">STABILIZED CELL LINE</span>
                  </div>
                  <div className="flex items-center justify-between">
                    <div>
                      <p className="font-serif text-xs text-stone-200 font-bold">
                        {currentTaskOrder.customerName || 'Aarav Sharma'}
                      </p>
                      <p className="font-mono text-xs text-amber-300 font-bold mt-0.5">
                        📞 Phone: {currentTaskOrder.customerPhone || '+91 98765 43210'}
                      </p>
                    </div>
                    <a 
                      href={`tel:${currentTaskOrder.customerPhone || '+91 98765 43210'}`} 
                      className="bg-emerald-600/20 hover:bg-emerald-600 text-emerald-400 hover:text-stone-100 p-2 rounded-lg border border-emerald-500/30 transition text-xs flex items-center justify-center gap-1 font-bold"
                    >
                ...
                <div className="bg-stone-900 p-3 rounded-xl space-y-3">
                  ...
                  <a 
                    href={`https://www.google.com/maps/search/?api=1&query=${encodeURIComponent(currentTaskOrder.address || '12, Heritage Lane, C-Scheme, Jaipur')}`}
                    target="_blank"
                    rel="noreferrer"
                    className="w-full bg-stone-950 text-stone-300 hover:text-white text-center py-2 rounded-lg font-bold border border-stone-800 hover:border-stone-700 transition block text-[10px] sm:text-xs"
                  >
                    🚀 Trigger Precise Compass Navigation in Google Maps App ↗
                  </a>
                </div>
              </div>
            )}
